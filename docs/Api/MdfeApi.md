...
```php
listarMdfe($cpf_cnpj, $ambiente, $top, $skip, $inlinecount, $referencia, $chave): \NuvemFiscal\Model\DfeListagem
```
...
try {
    $result = $apiInstance->listarMdfe($cpf_cnpj, $ambiente, $top, $skip, $inlinecount, $referencia, $chave);
    print_r($result);
} catch (Exception $e) {
