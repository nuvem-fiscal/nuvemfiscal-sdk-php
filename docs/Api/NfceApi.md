...
```php
listarLotesNfce($cpf_cnpj, $ambiente, $top, $skip, $referencia): \NuvemFiscal\Model\DfeLoteListagem
```
...
try {
    $result = $apiInstance->listarLotesNfce($cpf_cnpj, $ambiente, $top, $skip, $referencia);
    print_r($result);
} catch (Exception $e) {
...
```php
listarNfce($cpf_cnpj, $ambiente, $top, $skip, $referencia, $chave): \NuvemFiscal\Model\DfeListagem
```
...
try {
    $result = $apiInstance->listarNfce($cpf_cnpj, $ambiente, $top, $skip, $referencia, $chave);
    print_r($result);
} catch (Exception $e) {
